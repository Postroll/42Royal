...
export default function Create(func: Function){
    ...
    return (
        <context.Provider value={tmp}>
            <div className="flex h-screen w-screen pt-24 pb-8 px-8 bg-slate-800 gap-1">
                <form className="text-center text-black flex flex-col max-w-2/3 bg-slate-500 w-2/3 xsm:w-full xsm:max-w-full rounded-lg p-8 gap-2">
                    <h1 id='title' className='font-bold text-xl text-[#f5f5f5] mb-5 pointer-events-none'>Fill the form to create a problem</h1>
                    ...
                    <InputLimited placeholder="Example" defaultSize='1/10' value={example} onChange={(e) => setExample(e.target.value)}/>
                    <InputLimited placeholder="Contraints" value={constraints} onChange={(e) => setConstraints(e.target.value)}/>
